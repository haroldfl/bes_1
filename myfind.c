...
DIR *do_file (char* pPATH, enum Action *action, char* pArrArgument[],char* file_name){
...
DIR *do_dir ( char *pPATH, enum Action *action,char* pArrArgument[]) {
    ...
    if (pDIR == NULL) {
        ...
        while ((pdirent = readdir(pDIR)) != NULL) {
            ...
            strcat(newpath, "\0");
            do_file(newpath, action, pArrArgument, pdirent->d_name);
        }
        closedir(pDIR);
        }
    return NULL;
...
/// Function checks the action
///
/// Checks if the Action is a valid one
///
/// \param action Action to check
/// \retval 0 ... non valid action
/// \retval 1 ... valid action
int check_valid_action(enum Action action){
